...
BOOL CMyUtils::InitGlobalVariable()
{
	TCHAR	*p;

	// App path & Sytem path
	if (GetModuleFileName(NULL, g_strAppPath, _countof(g_strAppPath)) == 0)
		return FALSE;

	p = _tcsrchr(g_strAppPath, '\\');
	if (p)
		*p = 0;
	else
		return FALSE;

	if (GetSystemDirectory(g_strSysPath, _countof(g_strSysPath)) == 0)
		return FALSE;

	// Release version & Release date
	g_strVersion.LoadString(IDS_RELEASE_VERSION);
	g_strReleaseDate.LoadString(IDS_RELEASE_DATE);

	// Get Windows Version
//temp	g_iWinVersion = GetWindowsVersion();

	// Server IP
	CRegistry	reg;
	TCHAR		strRegName[64] = {0};
	int			iLen;

	memset(strRegName, 0, sizeof(strRegName));
	reg.ReadValue(_T("ServerIP"), strRegName, _countof(strRegName));
//temp	CMyUtils::Trim(strRegName, _T(" "));

	iLen = _tcslen(strRegName);
#ifdef UNICODE
	WideCharToMultiByte(CP_ACP, 0, strRegName, iLen, g_strServerIP, sizeof(g_strServerIP) - 1, NULL, NULL);
#else
	iLen = min(iLen, sizeof(g_strServerIP) - 1);

	strncpy(g_strServerIP, strRegName, iLen);
	g_strServerIP[iLen] = 0;
#endif
