			 ...
			 char * buf)
{
   ...
   if ( ! j)
   {
	 ...
	 if (p == 2)
	 {
	    if (buf[k] != '\0' && (unsigned)(buf[k] + 1) <= 256 && ! isprint(buf[k]))
	    {
	       j = 0;
	       break;
	    }
...
int ReadIdeDriveAsScsiDriveInNT (void)
{
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      TCHAR   driveName [256];
         ...
         //  Try to get a handle to PhysicalDrive IOCTL, report failure
         //  and exit if can't.
      _stprintf (driveName, _T("\\\\.\\Scsi%d:"), controller);
...
long getHardDriveComputerID (CString& psHardDriveSerialNumber)
{
   int done = FALSE;
   // char string [1024];
   __int64 id = 0;
   ...
   if (HardDriveSerialNumber [0] > 0)
   {
	  ...
	  psHardDriveSerialNumber = HardDriveSerialNumber;
...
long CDiskId32::getHardDriveID(CString &psHardDriveComputerID)
{
	return getHardDriveComputerID(psHardDriveComputerID);
}
